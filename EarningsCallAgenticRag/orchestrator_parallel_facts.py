...
# ---------- T+30 Configuration ------------
# Default return column in CSV. For T+30 evaluation, we try to get from PostgreSQL DB.
RETURN_HORIZON_DAYS = int(os.getenv("RETURN_HORIZON_DAYS", "30"))  # T+30 by default
RETURN_COLUMN_FALLBACK = "future_3bday_cum_return"  # Fallback if T+30 not available
...
def get_actual_return(row: pd.Series, ticker: str, quarter: str, horizon_days: int = 30) -> float:
    """Get actual post-earnings return for evaluation.

    Priority:
    1. PostgreSQL DB pct_change_t_plus_{horizon_days} (e.g., T+30)
    2. CSV fallback column (future_3bday_cum_return)

    Args:
        row: DataFrame row with earnings data
        ticker: Stock ticker
        quarter: Quarter string (e.g., "2024-Q1")
        horizon_days: Return horizon in trading days (default 30)

    Returns:
        Return as decimal (e.g., 0.05 for +5%)
    """
    # Parse quarter to year/q
    try:
        year, q = None, None
    ...
    # Try PostgreSQL DB first for accurate T+30 return
    if year and q:
        try:
            from pg_client import get_post_earnings_return, is_available
            if is_available():
                pg_return = get_post_earnings_return(ticker, year, q, days=horizon_days)
                if pg_return is not None:
                    # PostgreSQL returns percentage (e.g., 5.5), convert to decimal (0.055)
                    return pg_return / 100.0
        except Exception as e:
            pass  # Fall through to CSV fallback

    # Fallback to CSV column
    csv_return = row.get(RETURN_COLUMN_FALLBACK)
    if pd.notna(csv_return):
        return float(csv_return)
    ...
    try:
        ...
        for _, row in sector_df.iterrows():
            ...
            result_dict = {
                ...
                "research_note" : "",
                "actual_return" : get_actual_return(row, ticker, quarter, RETURN_HORIZON_DAYS),
                "error"         : "",
            }
...
def main() -> None:
    ...
    try:
        ...
        return
    # Use fallback column for initial filtering; actual returns are fetched via get_actual_return()
    if RETURN_COLUMN_FALLBACK in df.columns:
        df["returns"] = df[RETURN_COLUMN_FALLBACK]
    else:
        df["returns"] = 0.0  # Will be populated from PostgreSQL DB
    # df = df.iloc[878:]
    ...
    # Filter for significant returns (optional, commented out)
    """
    mask = (df[RETURN_COLUMN_FALLBACK] >= THRESHOLD) | \
           (df[RETURN_COLUMN_FALLBACK] <= -THRESHOLD)

    df = df.loc[mask].reset_index(drop=True)
    """

    # Drop entries where return data is unavailable (if column exists)
    if RETURN_COLUMN_FALLBACK in df.columns:
        df = df.dropna(subset=[RETURN_COLUMN_FALLBACK]).reset_index(drop=True)
